    ...
    <div className="bg-slate-900 rounded-xl p-5 border border-slate-800 shadow-lg">
      ...
      <div className="space-y-8">
        ...
        <div>
          <div className="flex justify-between items-end mb-3">
            ...
            <span className="text-3xl font-mono text-blue-400 font-bold">
              {height.toFixed(2)} <span className="text-lg text-slate-500">m</span>
            </span>
          </div>
        ...
        <div className="border-t border-slate-700 pt-6">
          <h3 className="text-lg font-bold text-slate-200 mb-4">对象参数调节</h3>
                ...
                <div className="grid grid-cols-1 gap-4">
                    ...
                    <div>
                        <div className="flex justify-between text-base mb-1">
                            <span className="text-slate-400">绳长 <Latex>L</Latex></span>
                            <span className="font-mono text-slate-200">{p.length.toFixed(2)}m</span>
                        </div>
                        <input 
                            type="range" min={0.5} max={4.0} step={0.1}
                            value={p.length}
                            onChange={(e) => onUpdatePendulum(p.id, { length: parseFloat(e.target.value) })}
                    ...
                    <div>
                        <div className="flex justify-between text-base mb-1">
                            <span className="text-slate-400">质量 <Latex>m</Latex></span>
                            <span className="font-mono text-slate-200">{p.mass.toFixed(1)}kg</span>
                        </div>
                        <input 
                            type="range" min={0.1} max={5.0} step={0.1}
                            value={p.mass}
                            onChange={(e) => onUpdatePendulum(p.id, { mass: parseFloat(e.target.value) })}
                            className="w-full h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-slate-400"
                        />
                    </div>
                </div>
