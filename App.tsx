...
export default function App() {
  ...
  const [pendulums, setPendulums] = useState<PendulumConfig[]>(INITIAL_PENDULUMS);
  ...
  const handlePendulumUpdate = (id: number, updates: Partial<PendulumConfig>) => {
    setPendulums(prev => prev.map(p => {
      if (p.id !== id) return p;
      return { ...p, ...updates };
    }));
  };
            ...
            <div className="space-y-8 text-lg text-slate-300 leading-relaxed">
                ...
                <section>
                    ...
                    <div className="bg-slate-950 p-4 rounded-lg border border-slate-800">
                        ...
                        <div>
                            水平方向：<Latex block>{String.raw`T \sin\theta = F_{n} = m \omega^2 r`}</Latex>
                        </div>
                    </div>
                </section>
                ...
                <section>
                    <h3 className="text-xl font-bold text-white mb-3 flex items-center gap-2">
                        <span className="w-6 h-6 rounded-full bg-blue-600 text-xs flex items-center justify-center">3</span>
                        联立求解
                    </h3>
                    <p className="mb-2 text-slate-400">
                        将水平方程除以竖直方程：
                    ...
                    <div className="bg-slate-950 p-4 rounded-lg border border-slate-800 space-y-3">
                        <Latex block>{String.raw`\tan\theta = \frac{m \omega^2 r}{mg} = \frac{\omega^2 r}{g}`}</Latex>
                        <p className="text-center text-sm text-slate-500 my-2">代入几何关系 <Latex>{String.raw`\tan\theta = r/h`}</Latex></p>
                        <Latex block>{String.raw`\frac{r}{h} = \frac{\omega^2 r}{g}`}</Latex>
                        <p className="text-center text-sm text-slate-500 my-2">消去 <Latex>r</Latex> (当 <Latex>r \neq 0</Latex>)</p>
                        <Latex block>{String.raw`\omega^2 = \frac{g}{h} \implies \omega = \sqrt{\frac{g}{h}}`}</Latex>
                    </div>
                </section>
                ...
                <section className="bg-indigo-900/20 border border-indigo-500/30 p-5 rounded-xl">
                    ...
                    <p>
                        圆锥摆的角速度 <Latex>\omega</Latex> <strong>只与垂直高度 <Latex>h</Latex> 有关</strong>。
                    </p>
